        ...
        match event {
            ...
            Event::RedrawRequested(_) => {
                 ...
                 */
                    ...
                    while (!game_over && !you_won){
                        let mut rng = rand::thread_rng();                    
                        let i: usize = rng.gen_range(2..sprites.len());
                        //until it hits the ground or platform, ball falls down
                        while (sprites[i].screen_region[1] - CELL_HEIGHT) > 30.0{ // TODO: add while not hitting the platform
                            sprites[i].screen_region[1] += 30.0;   
                        }
                    }
